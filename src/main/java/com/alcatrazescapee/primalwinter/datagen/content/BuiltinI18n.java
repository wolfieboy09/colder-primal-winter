...
public class BuiltinI18n extends LanguageProvider {
    private final Set<Block> untranslatedBlocks = PrimalWinterBlocks.BLOCKS.getEntries()
            .stream()
            .map(Supplier::get)
            .collect(Collectors.toSet());

    public BuiltinI18n(GatherDataEvent event) {
        super(event.getGenerator().getPackOutput(), PrimalWinter.MOD_ID, "en_us");
    }

    @Override
    protected void addTranslations() {
        add(PrimalWinter.MOD_ID + ".items", "Primal Winter");
        add(PrimalWinter.MOD_ID + ".subtitle.wind", "Wind Howls");
    ...
    private void addConfig(String key, String name) {
        add(PrimalWinter.MOD_ID + ".config." + key, name);
    }
}
